...
namespace ISpanShop.Models
{
	...
	public class DataSeeder
	{
		...
		private class SpecDimension
		{
			...
			public SpecDimension(string name, string[] values)
			{
				Name = name;
				Values = values;
			}
		...
		private static readonly Dictionary<string, (string Title, string Description)> ProductTranslationMap = new()
		{
			...
			{"Blue Frock", ("知性海軍藍連身洋裝", "氣質滿分的深藍色系，版型修身不挑身材，上班約會一件搞定的萬能戰衣。")},
			{"Girl Summer Dress", ("少女碎花涼感洋裝", "輕盈透氣的雪紡面料，搭配可愛碎花圖案，穿上就有度假好心情。")},
			{"Gray Dress", ("高級灰針織連身裙", "百搭中性灰色調，柔軟親膚的針織材質，隨意搭配外套就能展現都會時尚。")},
			{"Short Frock", ("俏皮荷葉邊短洋裝", "活潑的荷葉邊設計修飾手臂線條，短版裙擺展現青春活力。")},
